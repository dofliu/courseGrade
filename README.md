...
- **AI**：Google Gemini API（預設 `gemini-3.5-flash`，可在 app 內切換，如 `gemini-3.1-flash-lite` 省成本）
...
> **也可在 app 內設定**：「課程與配分管理」分頁有「AI 金鑰設定」卡，可直接填 `GEMINI_API_KEY` 並**切換模型**（預設 3.5 Flash，可選 3.1 Flash-Lite 等或自訂 id），存到本機 `edugrade-config.json`、即時生效。
> 優先序：API key 為**環境變數優先**；模型為 **app 設定優先**（不會被系統 `GEMINI_MODEL` 鎖死）。
...
| `GET /api/settings` | 金鑰狀態、目前模型與可選清單 |
| `POST /api/settings/gemini-key` | app 內設定 API key（寫本機 config、即時生效） |
| `POST /api/settings/gemini-model` | app 內切換模型（app 設定優先於環境變數） |
| `POST /api/analyze-file` | 單一檔案 AI 評分（資料夾批次用，可帶 rubric） |
| `POST /api/exam/generate` | 依講義／章節範圍 AI 出題（可指定難度分布／自動平衡總分） |
...
- `edugrade-config.json`：app 內設定的 API key／模型 — 已 gitignore。
